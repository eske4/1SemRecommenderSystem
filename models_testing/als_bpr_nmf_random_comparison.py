...
top_k_rec_random = pd.concat(recommendations, ignore_index=True)

# %% [markdown]
# ### Random Recommender metrics

# %%
random_ranking_metrics = get_ranking_results_python(test, top_k_rec_random)

random_diversity_metrics = get_diversity_results_python(train, top_k_rec_random)

random_results = generate_summary(train.size + test.size, "random", TOP_K, random_ranking_metrics, random_diversity_metrics)
...
df_results.loc[4] = random_results 

# %%
print(df_results.to_string())


df_results.to_csv('../results/als_vs_bpr_vs_nmf_vs_random.csv')
